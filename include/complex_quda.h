...
namespace quda
{
  ...
  template <typename ValueType> inline complex<ValueType> operator+(const complex<ValueType> &rhs);
  template <typename ValueType> inline complex<ValueType> operator-(const complex<ValueType> &rhs);
...
public:
  ...
  template <class X>
    ...
    template <typename T> inline complex<ValueType> &operator=(const complex<T> &z)
    {
      real(z.real());
      imag(z.imag());
      return *this;
...
public:
  ...
  template <typename T> inline complex<float> &operator=(const complex<T> &z)
  {
    real(z.real());
    imag(z.imag());
    return *this;
    ...
    inline void real(float re) { x = re; }
    inline void imag(float im) { y = im; }

    // cast operators
    inline operator std::complex<float>() const { return std::complex<float>(real(), imag()); }
    template <typename T> inline operator complex<T>() const
    {
...
public:
  ...
  template <typename T> inline complex<double> &operator=(const complex<T> &z)
  {
    real(z.real());
    imag(z.imag());
    return *this;
...
public:
  ...
  inline complex<int8_t> &operator+=(const complex<int8_t> &z)
  {
    real(real() + z.real());
    imag(imag() + z.imag());
    return *this;
...
public:
  ...
  inline complex<short> &operator+=(const complex<short> &z)
  {
    real(real() + z.real());
    imag(imag() + z.imag());
    return *this;
    ...
    inline void real(short re) { x = re; }
    inline void imag(short im) { y = im; }
...
public:
  ...
  inline complex<int> &operator+=(const complex<int> &z)
  {
    real(real() + z.real());
    imag(imag() + z.imag());
    return *this;
    ...
    inline void real(int re) { x = re; }
    inline void imag(int im) { y = im; }
...
(lhs.imag() * rhs.real() - lhs.real() * rhs.imag()) / rhs_norm);
  ...
  template <typename real> inline complex<real> cmul(const complex<real> &x, const complex<real> &y)
  {
    complex<real> w;
    w.x = x.real() * y.real();
    w.x -= x.imag() * y.imag();
  ...
  inline complex<real> cmac(const complex<real> &x, const complex<real> &y, const complex<real> &z)
  {
    complex<real> w = z;
    w.x += x.real() * y.real();
    w.x -= x.imag() * y.imag();
  ...
  inline auto cmac(const T1 &x, const T2 &y, const T3 &z)
  {
  ...
  template <typename real> inline complex<real> i_(const complex<real> &a)
  {
