...
#include <reduction_kernel_host.h>
  ...
  //imported from lattice_field.h
  /**
     @brief Helper function for setting auxilary string
     @return String containing location and compilation type
   */
  inline const char *compile_type_str_(QudaFieldLocation location = QUDA_INVALID_FIELD_LOCATION)
  {
  ...
  */
  ...
  protected:
    ...
    */
    ...
    template <template <typename> class Functor, typename T, typename CommReducer = comm_reduce_null<T>,
  ...
  public:
    ...
    {
      u64toa(vol, n_items);
      strcpy(aux, compile_type_str_(location));
  ...
  public:
    TunableMultiReduction(size_t n_items, unsigned int n_batch, unsigned int n_batch_block_max = 1u, QudaFieldLocation location = QUDA_INVALID_FIELD_LOCATION) :
